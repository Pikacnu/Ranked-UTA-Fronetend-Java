...
public class DamagedCommand implements ICommand {
	...
	private static class ExecuteCommand implements Command<ServerCommandSource> {
		...
		public int run(CommandContext<ServerCommandSource> context) {
			 ...
			 * {
			 * // Get the arguments
			 * String target = EntityArgumentType.getEntity(context,
			 * "target").getUuidAsString();
			 * String attacker;
			 * 
			 * try
			 * {
			 * attacker = EntityArgumentType.getEntity(context,
			 ...
			 * WebSocketClient.sendMessage(new Message(Action.damage,
			 * WebSocketClient.serverSessionId, payload));
			 * 
			 * source.sendMessage(Text.literal("Player damaged event sent successfully!").
			 * withColor(0x00FF00));
			 * }
			 * catch (Exception e)
			 * {
			 * source.sendError(Text.literal("Failed to send player damaged event!").
			 * withColor(0xFF0000));
